...
for ($l = $lines; $l > 2; $l--) {
    ...
    for ($i = 1; $i < $l; $i++)
        if ((floor($cap / $l) * $i) % 10 > $maxfrac)
            $maxfrac = (floor($cap / $l) * $i) % 10;
...
for ($i = 0; $i < $slotCount; $i++) {
    ...
    if ($txData > 0) {
        ...
        imagefilledrectangle($im, round($x1), round($y1), round($x2), round($y2), $blue);
    }
    ...
    if ($rxData > 0) {
        ...
        imagefilledrectangle($im, round($x1), round($y1), round($x2), round($y2), $green);
    }
}
